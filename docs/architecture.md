...
- Data Export v1 is a read-only CSV vertical slice under More at `/data-export`. It exports transactions, accounts, one budget month, recurring rules, credit-card statements, and report-summary metrics through Android's native share UI. Transaction rows use bounded keyset batches and incremental file writes; existing financial services remain authoritative. CSV is explicitly not a backup or restore format. See [data-export.md](data-export.md).
...
- Local Notifications is an optional Android-only device slice under More at `/notifications-settings`. A protected runtime creates stable channels, reconciles recurring and daily schedules only after unlock/database initialization, evaluates budget crossings from existing budget read models, and validates notification taps behind App Lock. Device metadata is stored in migration 0006 and excluded from logical backups. See [notifications.md](notifications.md).
- Credit Card Management v1 adds account cycle metadata, preserved statement history, a dedicated card route, guided payments through normal transfers, utilization guidance, opt-in reminders, and backup format v2. See [credit-cards.md](credit-cards.md).
- Linked Refunds v1 adds a fourth transaction type, atomic expense links, inherited category attribution, net budget spending, credit-card debt reduction without payment attribution, and backup format v3. See [linked-refunds.md](linked-refunds.md).
- Multi-Currency v1 adds per-account COP/USD currency, a centralized currency/money/exchange-rate domain, a Frankfurter USD/COP reference-rate cache with manual fallback, per-transaction COP base-amount snapshots, dual-leg cross-currency transfers, USD income/expense/refund/cards, backup format v4, and native+COP CSV columns. COP remains the fixed base currency for all consolidated reporting. See [ADR 0005](decisions/0005-multi-currency-cop-usd.md), migration `0009_multi_currency`, and the multi-currency notes in [data-model.md](data-model.md) and [financial-rules.md](financial-rules.md).
- Investments v1 is a functional vertical slice under More at `/investments` (also surfaced from Accounts and a Home card). It adds an `investment` account type with 1:1 metadata and manual valuation history, tracking value by total balance. Contributions/withdrawals are ordinary transfers; a valuation update changes net worth (current value = net contributions + latest unrealized gain) without creating any transaction, and net worth uses current value for investment accounts with no double counting. It advances backup to format v5, adds two CSV exports and a Reports investment section, and requires migration `0012_investments_v1` (create-copy-swap to add the account type). See [investments.md](investments.md) and [ADR 0006](decisions/0006-investments-v1.md).
- Expense subcategories add a second, optional classification level. `categories` gains a self-referencing `parent_category_id` (depth capped at two) and the three transaction-shaped tables gain a nullable `subcategory_id`; `category_id` keeps holding the top-level category, so Budgets, Reports, notifications, filters and CSV keep their meaning with no re-computation. Selecting a subcategory infers its parent in `TransactionService`, so every write path shares one implementation. Reports group at the leaf and fold upward, giving an expandable breakdown whose rows sum to the category total by construction. It advances backup to format v6 and requires migration `0013_category_subcategories`, which is purely additive — no table is rebuilt. See [ADR 0007](decisions/0007-category-subcategories.md).
- An overall monthly ceiling sits above the per-category budgets: one optional base-currency limit per month, counting every posted expense minus refunds rather than one category subtree, so spending in unbudgeted categories is finally visible. It is a separate `monthly_budgets` table rather than a nullable `category_id` on `budgets` — the ceiling has no category, no nesting and no shared query — so migration `0015_monthly_budget_ceiling` only creates a table. The ceiling carries forward to later months instead of materializing rows, and an inactive row is the tombstone that stops it. The backup collection is optional and the logical format stays v7. See [ADR 0009](decisions/0009-overall-monthly-ceiling.md).
- First-run onboarding: a fresh install opens a welcome flow before the tabs, where the user picks the base currency (suggested from the device locale) or restores a backup. The base locks with the first transaction or budget, so it has to be chosen before either can exist. Completion is `app_settings.onboarding_completed_at` (migration `0016_onboarding_completion`, additive, backfilled for installs with data) and is read synchronously like the base currency, primed during database initialization and again after a restore. The `(tabs)` layout redirects to `/onboarding` until it is set.
- Interface language: six typed message catalogs under `src/i18n/locales` (English source; Spanish, Brazilian Portuguese, French, German, Italian typed against it) with no i18n dependency. The active language follows the device (`expo-localization`) or a choice saved in `expo-sqlite/kv-store`, so it is available before the financial database opens and stays out of backups. The root `Stack` is keyed by language, so a change remounts every screen instead of leaving memoized text behind. Changing it renames untouched default categories and rebuilds scheduled notifications. See [translations.md](translations.md).
...
- Migration 0007 adds card cycle days, statement history, card notification preferences, and the card-reminder scheduling domain without changing transaction rows.
- Migration 0008 safely rebuilds transactions with nullable `original_transaction_id`, preserves split and recurring-occurrence links, and installs refund lifecycle guards.
...
Data Export likewise remains outside the five primary tabs. More pushes `/data-export`; the route is a thin composition boundary over the export hook/service/repository/file adapter. The screen executes no SQL and implements no CSV escaping. The complete route remains inside App Lock and database initialization gates.
...
Notifications likewise remains outside the five primary tabs. More pushes `/notifications-settings`; its route uses notification services and repositories rather than raw Expo or SQLite APIs. The root notification runtime is nested inside the App Lock and database gates, so no financial coordinator mounts before unlock.
