...
Fresh install → create accounts/categories → expense/income/transfer → edit/void → budgets → recurring generate/post → reports → credit card + statement + min/remaining payment → partial/full refund → refund a card purchase → verify Home/Accounts/Transactions/Budgets/Reports/card details → backup → CSV export → App Lock enable → background/resume → restore → notification reconciliation → light + dark theme → small-screen layout → keyboard → screen-reader labels → restart persistence → no stale values after mutations.
